...
app.layout = html.Div(
    [   html.Div(className='container-fluid', children=[html.H2('Live Twitter Sentiment', style={'color':"#CECECE"}),
                                                        html.H5('Fortune 500 Brand:', style={'color':app_colors['text']}),
                                                  dcc.Input(id='sentiment_term', value='amazon', type='text', style={'color':app_colors['someothercolor']}, disabled = True),
                                                  ],
...
# making a counter with blacklist words and empty word with some big value
stop_words.append('')
blacklist_counter = Counter(dict(zip(stop_words, [1000000]*len(stop_words))))

# compiling a regex for split operations (punctuation list, plus space and new line)
